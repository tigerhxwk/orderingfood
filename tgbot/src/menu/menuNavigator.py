...
class menuNav:
    ...
    def __addToCurrPrintCount (self, chatId, value = 10):
        if chatId in self.__BreadCrumbs.keys():
            currcount = self.__BreadCrumbs[chatId]['currPrintCount']
            self.__innerDebug(f'adding sent {value} to printcount {currcount}')
            self.__BreadCrumbs[chatId]['currPrintCount'] += value
            currcount = self.__BreadCrumbs[chatId]['currPrintCount']
            self.__innerDebug(f'new value  {currcount}')

    def __rmFromCurrPrintCount (self, chatId, value):
        if chatId in self.__BreadCrumbs.keys():
            currcount = self.__BreadCrumbs[chatId]['currPrintCount']
            self.__innerDebug(f'removing deleted {value} from printcount {currcount}')
            self.__BreadCrumbs[chatId]['currPrintCount'] -= value
            currcount = self.__BreadCrumbs[chatId]['currPrintCount']
            self.__innerDebug(f'new value  {currcount}')
